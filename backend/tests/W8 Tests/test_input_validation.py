...
def test_manager_chat_rejects_whitespace_message():
    response = client.post(
        "/api/agents/manager/chat",
        json={"message": "   ", "post_id": "post-1"},
    )
...
def test_memory_store_rejects_empty_content():
    response = client.post("/api/memory/store", json={"content": "", "tags": []})
    assert response.status_code == 422


def test_memory_store_rejects_whitespace_content():
    response = client.post("/api/memory/store", json={"content": "   ", "tags": []})
    assert response.status_code == 422
